...
const ModalProductEdit = ({ isOpen, onClose, product, updateProduct }) => {
    const [descriptions, setDescriptions] = useState(product?.descriptions || [{ title: '', content: '' }]);
    const [selectedColors, setSelectedColors] = useState(product?.colors || []);
    const [images, setImages] = useState(
        product?.thumbnails?.map((url) => ({ file: null, preview: url, caption: '' })) 
        || [{ file: null, preview: '', caption: '' }]
    );
    ...
    console.log("dateAdded", product.dateAdded);
    ...
    const handleSubmit = () => {
        ...
        // Lọc ra các ảnh mới (có file thực sự)
        const newImageFiles = images.filter(img => img.file instanceof File).map(img => img.file);
        // Lọc ra các ảnh cũ (chỉ có URL, không có file)
        const oldImageURLs = images.filter(img => !(img.file instanceof File)).map(img => img.preview);
    
        // Cập nhật sản phẩm với ảnh mới
        console.log("Cập nhật sản phẩm với các ảnh mới:", newImageFiles);
        console.log("Giữ lại các ảnh cũ:", oldImageURLs);
    
        // Gửi dữ liệu lên server
        if (newImageFiles.length > 0 || oldImageURLs.length > 0) {
            updateProduct(productToUpdate, newImageFiles, oldImageURLs); // Gửi ảnh mới và giữ lại ảnh cũ
        } else {
            updateProduct(productToUpdate); // Nếu không có thay đổi về ảnh
        }
        onClose();
    };    
    ...
    return (
        ...
        <div className="relative bg-white border border-gray-200 p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto animate-fadeIn transition-transform duration-300"> 
            ...
            <div className="space-y-4 mt-2">
                ...
                {/* Ngày nhập */}
                <div>
                    <label className="block font-medium mb-1 text-gray-700">Ngày nhập</label>
                    <input 
                    type="date" 
                    name="dateAdded" 
                    value={formData.dateAdded ? formData.dateAdded.split('T')[0] : ''} // Chuyển đổi định dạng
                    onChange={handleChange}
                    required 
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200" 
                    />
