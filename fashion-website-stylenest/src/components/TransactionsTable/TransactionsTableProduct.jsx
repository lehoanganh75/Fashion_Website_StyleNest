...
const TransactionsTableProduct = ({
  products,
  saveProduct,
  updateProduct,
  deleteProduct,
    ...
    <div className="bg-white border border-gray-300 p-4.5 rounded-lg shadow-sm max-w-full font-['Roboto']">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="relative flex items-center w-full max-w-lg gap-2 mx-4">
          ...
          <button
            className="px-4 py-2 bg-white text-gray-600 text-sm border border-gray-300 rounded-lg shadow-sm transition"
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              <th className="text-left pb-4 pl-4 font-medium text-gray-500">
                Sản phẩm
              </th>
              ...
              <tr key={products.id} className="border-b border-gray-200 h-16">
                <td className="py-4 pl-4">
                  <div className="flex items-center">
                    <div
                      className={
                        "text-white w-10 h-10 rounded-full flex items-center justify-center mr-3"
                      }
                    >
                      <img
                ...
                <td>
                  <span
                    className={`px-4 py-2 rounded-full text-sm ${changeColor(
                      products.condition
                    )}`}
                  >
          ...
          {pages.map((page, index) => {
            ...
            if (page <= 3 || page >= pages.length - 2 || page === currentPage) {
              ...
              return (
                <button
                  key={index}
                  className="w-10 h-10 flex items-center justify-center rounded-lg text-gray-600"
                >
                  ...
                </button>
              );
      ...
      {dropdownOpen && selectedProducts && (
        ...
        >
          <button
            onClick={() => {
              console.log(
                `View more details for products: ${selectedProducts.id}`
              );
              setDropdownOpen(false);
              setIsModalOpen(true);
            }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center font-['Roboto']">
          ...
          <div className="relative bg-white border border-gray-200 p-6 md:p-8 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto animate-fadeIn transition-transform duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-gray-700 text-sm sm:text-base mb-6">
              <p>
                <strong>Thương hiệu:</strong> {selectedProducts.brand}
              </p>
              <p>
                <strong>Ngày nhập:</strong>{" "}
                {formatDate(selectedProducts.dateAdded)}
              </p>
              <p>
                <strong>Loại:</strong> {selectedProducts.type}
              </p>
              <p>
                <strong>Giảm giá:</strong> {selectedProducts.discount}%
              </p>
              <p>
                <strong>Giá tiền:</strong>{" "}
                {formatCurrency(selectedProducts.price)}
              </p>
              <p>
                <strong>Màu sắc:</strong> {selectedProducts.colors}
              </p>
              <p>
                <strong>Kích thước:</strong> {selectedProducts.size}
              </p>
              <p>
                <strong>Trạng thái:</strong>{" "}
                {selectedProducts.condition ? "Hàng mới" : "Hàng cũ"}
              </p>
              <p>
                <strong>SKU:</strong> {selectedProducts.SKU}
              </p>
              <p>
                <strong>Đánh giá:</strong> {selectedProducts.rating} ⭐
              </p>
              <p>
                <strong>Tồn kho:</strong> {selectedProducts.instock}
              </p>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="border border-gray-300 rounded-lg p-4 bg-white shadow-sm text-sm sm:text-base text-gray-700 space-y-2 text-justify">
                <p>
                  <strong>Vật liệu:</strong> {selectedProducts.details.material}
                </p>
                <p>
                  <strong>Vải vóc:</strong> {selectedProducts.details.fabric}
                </p>
                <p>
                  <strong>Xuất xứ:</strong> {selectedProducts.details.origin}
                </p>
                <p>
                  <strong>Hướng dẫn bảo quản:</strong>{" "}
                  {selectedProducts.details.careInstructions}
                </p>
              </div>
            </div>
