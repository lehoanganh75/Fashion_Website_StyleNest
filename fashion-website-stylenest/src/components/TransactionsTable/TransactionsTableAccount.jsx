...
const TransactionsTableAccount = ({
  accounts,
  saveAccount,
  updateAccount,
  deleteAccount,
  ...
  const handleSaveChanges = () => {
    const updatedAccount = {
      id: checked
        ? Math.max(...accounts.map((a) => a.id || 0), 0) + 1
        : selectedorders.id,
      userName,
      password,
      email,
    ...
    <div className="bg-white border border-gray-300 p-4.5 rounded-lg shadow-sm max-w-full font-['Roboto']">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {paginatedaccounts.map((accounts, index) => (
              <tr key={index} className="border-b border-gray-200 h-16">
                ...
                <td className="text-gray-500">
                  {accounts.role == "customer" ? "Khách hàng" : "Admin"}
                </td>
                <td>
                  <button
      ...
      {dropdownOpen && selectedorders && (
        ...
        >
          <button
            onClick={() => {
              console.log(
                `View more details for account: ${selectedorders?.id}`
              );
              setDropdownOpen(false);
